...
module Attention = struct
  let multihead vs (config : Config.t) =
    ...
    fun ~query ~key ~value ~mask ~is_training ->
      ...
      let q = Tensor.div_scalar q (Scalar.float sqrt_dim_per_head) in
      ...
      let scores =
        ...
        | Some mask ->
          let mask =
            Tensor.le_tensor mask (Tensor.add_scalar (Tensor.zeros_like mask) (Scalar.f 0.1))
  ...
  let question_answering vs (config : Config.t) ~qa_dropout_p =
    ...
    fun xs ~mask ~is_training ->
      ...
      | [ start_logits; end_logits ] ->
        let start_logits = Tensor.squeeze_dim start_logits ~dim:(-1) in
        let end_logits = Tensor.squeeze_dim end_logits ~dim:(-1) in
        start_logits, end_logits
      | _ -> assert false
end
